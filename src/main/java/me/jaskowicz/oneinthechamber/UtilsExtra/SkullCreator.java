 ...
 *
 * @author Dean B on 12/28/2016. Archie Jaskowicz edited at 02/07/2020
 */
public class SkullCreator {
    ...
    public static ItemStack itemWithBase64(ItemStack item, String base64) {
        ...
        String intArray = UUIDIntoIntArray(random.toString());

        System.out.println(random);
        System.out.println(random.toString());
        System.out.println(intArray);

        System.out.println("{SkullOwner:{Id:" + intArray + ",Properties:{textures:[{Value:\"" + base64 + "\"}]}}}");

        return Bukkit.getUnsafe().modifyItemStack(item,
                "{SkullOwner:{Id:" + intArray + ",Properties:{textures:[{Value:\"" + base64 + "\"}]}}}"
        );
    }
    ...
    // uuid e.g. "11bf5b37-e0b8-42e0-8dcf-dc8c4aefc000"
    private static String UUIDIntoIntArray(String uuid) {
        String[] uuidComponents = uuid.split("-"); //Split on "-" char
        // string[0] = "11bf5b37", string[1] = "e0b8" etc

        String fullUUID = "";

        // put it back together in one string, e.g. "11bf5b37e0b842e08dcfdc8c4aefc000"
        for(String str : uuidComponents) {
            fullUUID += str;
        }

        // Now start parsing each 8 characters:
        //int[] result = new int[4];

        String intarray = "[I;";

        for(int i = 0; i < 4; i++) {
            String partialUUID = fullUUID.substring(i*8, i*8+8); // get 32 bit blocks

            System.out.println(partialUUID);

            if(i != 3) {
                intarray += ((int) Long.parseLong(partialUUID, 16)) + ","; // do whatever string parsing

            } else {
                intarray += ((int) Long.parseLong(partialUUID, 16)) + "]";
            }
        }
